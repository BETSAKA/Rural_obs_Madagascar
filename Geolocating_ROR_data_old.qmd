...
locations <- tibble(
  ...
  name = c("Antalaha", "Antsirabe", "Marovoay", "Toliara littoral", "Antsohihy", 
           "Tsiroanomandidy", "Farafangana", "Ambovombe", 
           "Alaotra", "Manjakandriana", "Toliara Nord", 
           "Fenerive Est", "Bekily", "Mahanoro", "Itasy", 
...
Example for the evaluation of protected areas.
...
ggplot(obs_PA, aes(x = survey_years, y = facet_label)) +
  geom_point(aes(x = PA_creation_year), color = "red", size = 3)+
  geom_point(color = "blue") +
  # geom_point(data = obs_PA %>% 
  #              group_by(facet_label) %>% 
  #              summarise(PA_creation_year = first(PA_creation_year)),
  #            aes(x = PA_creation_year, y = 1), 
  #            size = 3, color = "red") +
...
obs_PA <- obs_PA %>%
  mutate(test = PA_Name %in% Vahatra$nom)

# Step 1: Fetch Survey Years and add as a new column in obs_PA
obs_PA <- obs_PA %>%
  rowwise() %>%
  mutate(
    survey_years = list(
...
for (i in 1:nrow(obs_PA)) {
    current_data <- obs_PA[i, ]
    
    # Create a plot for each Observatory and PA
    p <- ggplot() +
        # Plot for survey years
        geom_point(data = obs_count %>% filter(observatory_name == current_data$Observatory_name),
                   aes(x = year, y = 1), color = "blue") +
        # Plot for PA creation year - ensure to reference current_data
        geom_point(data = data.frame(x = current_data$PA_creation_year, y = 1),
...
# Merging obs_PA with obs_count
obs_data <- obs_PA %>%
  left_join(obs_count %>%
              select(observatory_name, survey_year = year), 
            by = c("Observatory_name" = "observatory_name"))

# Merging obs_PA with Vahatra to get PA creation year
obs_data <- obs_data %>%
  left_join(Vahatra %>%
              st_drop_geometry() %>%
              select(PA_Name = nom, PA_creation_year = an_crtn), 
            by = "PA_Name")

# Adjusting the data for y-axis labels
obs_data$observatory_PA <- paste(obs_data$Observatory_name, "-", obs_data$PA_Name)

# Get a list of all unique observatory-PA combinations
unique_observatory_PA <- unique(obs_data$observatory_PA)

# Initialize an empty list to store the plots
plot_list <- list()

# Loop through each unique observatory-PA and create a plot
for (observatory_PA in unique_observatory_PA) {
  # Filter data for the current observatory-PA
  current_data <- obs_data %>%
    filter(observatory_PA == observatory_PA)
  
  # Create the plot
  p <- ggplot(current_data, aes(x = survey_year, y = observatory_PA)) +
    geom_point(color = "blue") +
    geom_vline(data = current_data %>% distinct(Observatory_name, PA_Name, PA_creation_year),
               aes(xintercept = PA_creation_year), color = "red", linetype = "dashed") +
    scale_x_continuous(limits = c(1990, 2020)) +
    labs(x = "Year", y = "Observatory and PA") +
    theme_minimal() +
    theme(axis.title.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank())
  
  # Add the plot to the list
  plot_list[[observatory_PA]] <- p
}
...
We visually explore the the georeferenced data to observe its spatial distribution and spot eventual anomalies or clustering patterns that 