...
# Get the list of yearly folders
year_folders <- list.dirs(base_path, full.names = TRUE, recursive = FALSE)

# Function to read .dta file names from a given directory
read_dta_filenames <- function(path) {
  list.files(path, pattern = "\\.dta$", full.names = FALSE) %>%
    unique()
}

# Create a data frame with file names and corresponding years
files_years_df <- map_df(year_folders, function(folder) {
  year <- basename(folder)
  filenames <- read_dta_filenames(folder)
  tibble(filename = filenames, year = year)
}, .id = "folder") %>%
  group_by(filename) %>%
  summarise(years = toString(unique(year))) %>%
  ungroup()

# Load the Excel file
file_labels <- read_excel("references/file_labels.xlsx")
file_labels <- left_join(files_years_df, file_labels, by = c("filename" = "file"))

writexl::write_xlsx(file_labels, "references/file_labels2.xlsx")
