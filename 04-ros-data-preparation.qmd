...
### Anonymization of survey respondents
...
Labels are included for all variables in the raw data, but not for the tables. We recoded them manually from the questionnaires and we include them in the STATA files. 
...
The ROS survey data was natively entered and managed in STATA, which is a proprietary format. To facilitate the processing by all user, we also publish the data in an open format (tabulation-separated values, tsv).
